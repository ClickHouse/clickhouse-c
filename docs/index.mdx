...
`clickhouse-c` is a header-only C client for the ClickHouse [native protocol](/reference/interfaces/specs/NativeProtocol), carrying [Native format](/reference/interfaces/specs/NativeFormat) blocks.
...
TCP clients support ClickHouse 23.3 and newer (server revision 54462 or higher).
...
int rc = chc_client_init(&client, &opts, &al, &io, &exc, &err);
if (rc != CHC_OK) {
    fprintf(stderr, "connect: %s\n", rc == CHC_ERR_SERVER ? exc->display_text
                                                          : err.msg);
...
Send the query, then drain packets until `CHC_PKT_END_OF_STREAM` or `CHC_PKT_EXCEPTION`. Use `chc_client_send_query_ex` to
...
for (;;) {
    ...
    bool done = pkt.kind == CHC_PKT_END_OF_STREAM || pkt.kind == CHC_PKT_EXCEPTION;
...
`chc_client_recv_packet`. Transport, protocol, and decode failures return non-OK.
An exception terminates a query without a following EndOfStream packet. The first `CHC_PKT_DATA`
...
| `CHC_COL_NOTHING` | `chc_column_n_rows(c)`; `Nothing` and `Void` have no value data to access |
...
void print_value(const chc_type *t, const chc_column *c, size_t row)
{
    ...
    case CHC_COL_FIXED: {
        ...
        size_t es;
        const uint8_t *p = chc_column_fixed_data(c, &es);
        p += row * es;
...
Reuse `qopts` from [Running a query](#running-a-query) to pin text type names.
...
chc_type *u64 = NULL;
if (chc_type_parse("UInt64", 6, &al, &u64, &err) != CHC_OK) {
...
uint64_t offsets[3] = { 5, 11, 19 };   /* "hello", "buenas", "goedendag" */
const uint8_t bytes[] = "hellobuenasgoedendag";
chc_type *string = NULL;
if (chc_type_parse("String", 6, &al, &string, &err) != CHC_OK) {
    fprintf(stderr, "type: %s\n", err.msg);
...
int insert_rc = chc_client_send_data(client, &bb, &err);
if (insert_rc == CHC_OK)
    insert_rc = chc_client_send_data(client, NULL, &err);
...
/* Drain until completion or exception. */
for (;;) {
    chc_packet pkt = {};
    if (chc_client_recv_packet(client, &pkt, &err) != CHC_OK) {
        fprintf(stderr, "recv: %s\n", err.msg);
    ...
    bool failed = pkt.kind == CHC_PKT_EXCEPTION;
    bool done = pkt.kind == CHC_PKT_END_OF_STREAM;
    if (failed) fprintf(stderr, "server: %s\n", pkt.exception->display_text);
    chc_packet_clear(client, &pkt);
    if (failed) return 1;
    if (done) break;
}
...
if (chc_async_client_init(&c, &opts, &al, &err) != CHC_OK) {
...
Handshake rejection returns `CHC_ERR_SERVER` and leaves `err.msg` empty. When `exc` is not NULL,
caller owns `*exc` and frees it with `chc_exception_free`. Pass `NULL` to discard exception details.
...
- `UUID`, `IPv4`, `IPv6`
- `Enum8`, `Enum16`
- `Nullable(T)`, `Array(T)`, `Tuple(...)`, `Map(K, V)`; `Nested(...)` as `Array(Tuple(...))`
- `LowCardinality(T)`
- `Nothing`, `Void`
- `Interval`
- `QBit(...)`
...
one JSON document in a `CHC_COL_STRING` column. Hints and typed paths in the type name,
as in ``JSON(max_dynamic_paths=16, `a.b` UInt32, SKIP `a.e`)``, are skipped: they steer
the server's dynamic storage, not the string serialization. Build same shape with `chc_build_string`;
